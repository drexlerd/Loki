...
TEST(LokiTests, TypedListOfVariablesRecursivelyTest) {
    ...
    EXPECT_NO_THROW(parse_ast("?kilo ?liko - loki", typed_list_of_variables_recursively(), ast));
    EXPECT_EQ(parse_text(ast), " ?kilo ?liko - loki");  // printer adds leading whitespace

    EXPECT_ANY_THROW(parse_ast("kilo liko", typed_list_of_variables_recursively(), ast));
    EXPECT_ANY_THROW(parse_ast("?kilo ?liko", typed_list_of_variables_recursively(), ast));
}
