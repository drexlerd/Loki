...
TEST(Loki2ParserSuite, JsonSuiteCoversEveryBenchmarkProblem)
{
    const auto suite_value = ygg::common::load_json_file(suite_root_path() / "tests/unit/loki2/parser_suite.json");
    const auto& suite = ygg::common::as_object(suite_value, "suite");
    const auto prefix = ygg::common::find_string(suite, "prefix", "suite").value_or("");
...
TEST(Loki2ParserSuite, ParsesAllBenchmarkDomainAsts)
{
...
TEST(Loki2ParserSuite, ParsesAllBenchmarkTaskAsts)
{
    const auto cases = load_cases();
    ASSERT_FALSE(cases.empty());
...
TEST(Loki2ParserSuite, ParsesAndReparsesRepresentativeBenchmarkDomains)
{
...
TEST(Loki2ParserSuite, ParsesAndReparsesRepresentativeBenchmarkTasks)
{
    const auto root = fs::path(std::string(DATA_DIR)) / "planning-benchmarks" / "tests";
