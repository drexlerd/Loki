...
TEST(LokiTests, LokiPddlAstNumberTest)
{
    ast::Number ast;

    EXPECT_NO_THROW(parse_ast("5", number(), ast));
    EXPECT_EQ(string(ast, DefaultFormatter {}), "5");
    EXPECT_NO_THROW(parse_ast("4.2", number(), ast));
    EXPECT_EQ(string(ast, DefaultFormatter {}), "4.2");
    EXPECT_NO_THROW(parse_ast("6 7", number(), ast));
    EXPECT_EQ(string(ast, DefaultFormatter {}), "6");
    // TODO: Is this really what we want?
    EXPECT_NO_THROW(parse_ast("1loki", number(), ast));
    EXPECT_EQ(string(ast, DefaultFormatter {}), "1");
