...
def make_number_expression(repository, value):
    ...
    return build(repository, pypddl.FunctionExpressionBuilder.number(ix(number)))
...
def test_repository_constructs_domain_and_task_programmatically():
    ...
    variable_term = build(repository, pypddl.TermBuilder.variable(ix(variable)))
...
def test_repository_constructs_numeric_function_task_bits():
    ...
    term = build(repository, pypddl.TermBuilder.variable(ix(variable)))
    ...
    condition = build(repository, pypddl.ConditionBuilder.numeric_constraint(ix(numeric_node)))
    ...
    effect = build(repository, pypddl.EffectBuilder.numeric(ix(numeric_effect_node)))
...
def test_semantic_error_exposes_code_and_source_range():
    parser = pypddl.Parser()
    try:
        parser.parse_domain("""
...
""")
        ...
        assert error.code == pypddl.SemanticErrorCode.DuplicatePredicate
        assert error.source_range is not None
        assert error.source_range["begin"]["line"] >= 1
        assert "Duplicate predicate" in str(error)
    else:
        raise AssertionError("expected duplicate predicate diagnostic")


def test_parse_error_exposes_code_and_source_range():
    parser = pypddl.Parser()
    try:
        parser.parse_domain("(define (domain broken) (:predicates (p))")
    except pypddl.SemanticError as error:
        assert error.code == pypddl.SemanticErrorCode.ParseFailure
        assert error.source_range is not None
        assert error.source_range["begin"]["line"] >= 1
    else:
...
""")
    ...
    assert action.get_precondition().get_kind() == "or"
    assert isinstance(action.get_precondition().get_value(), pypddl.ConditionOr)
    assert [child.get_kind() for child in action.get_precondition().get_value().get_conditions()] == ["literal", "literal"]
    assert action.get_effect().get_kind() == "and"
    assert isinstance(action.get_effect().get_value(), pypddl.EffectAnd)
    assert action.get_effect().get_value().get_effects()[0].get_kind() == "when"
    assert isinstance(action.get_effect().get_value().get_effects()[0].get_value(), pypddl.EffectWhen)
...
def test_numeric_expression_variant_views_are_inspectable():
    ...
    assert number.get_kind() == "number"
    assert isinstance(number.get_value(), pypddl.FunctionExpressionNumber)
    assert number.get_value().get_value() == 2.0
...
def test_repository_exposes_recursive_constructors_and_accessors():
    ...
    condition_not = build(repository, pypddl.ConditionBuilder.negation(ix(condition_not_node)))
    condition_or_node = build(repository, pypddl.ConditionOrBuilder(ixs([base_condition, condition_not])))
    condition_or = build(repository, pypddl.ConditionBuilder.disjunction(ix(condition_or_node)))
    condition_imply_node = build(repository, pypddl.ConditionImplyBuilder(ix(base_condition), ix(condition_or)))
    condition_imply = build(repository, pypddl.ConditionBuilder.implication(ix(condition_imply_node)))
    condition_exists_node = build(repository, pypddl.ConditionExistsBuilder(ixs([parameter]), ix(condition_imply)))
    condition_exists = build(repository, pypddl.ConditionBuilder.existential(ix(condition_exists_node)))
    condition_forall_node = build(repository, pypddl.ConditionForallBuilder(ixs([parameter]), ix(condition_exists)))
    condition_forall = build(repository, pypddl.ConditionBuilder.universal(ix(condition_forall_node)))
    ...
    unary = build(repository, pypddl.FunctionExpressionBuilder.unary(ix(unary_node)))
    ...
    binary = build(repository, pypddl.FunctionExpressionBuilder.binary(ix(binary_node)))
    multi_node = build(repository, pypddl.MultiFunctionExpressionBuilder(pypddl.MultiArithmeticOperator.Add, ixs([number, binary])))
    multi = build(repository, pypddl.FunctionExpressionBuilder.multi(ix(multi_node)))
    ...
    numeric_effect = build(repository, pypddl.EffectBuilder.numeric(ix(numeric_effect_node)))
    ...
    effect_when = build(repository, pypddl.EffectBuilder.conditional(ix(effect_when_node)))
    effect_forall_node = build(repository, pypddl.EffectForallBuilder(ixs([parameter]), ix(effect_when)))
    effect_forall = build(repository, pypddl.EffectBuilder.universal(ix(effect_forall_node)))
    effect_one_of_node = build(repository, pypddl.EffectOneOfBuilder(ixs([literal_effect, numeric_effect])))
    effect_one_of = build(repository, pypddl.EffectBuilder.one_of(ix(effect_one_of_node)))
    ...
    effect_probabilistic = build(repository, pypddl.EffectBuilder.probabilistic(ix(effect_probabilistic_node)))
    ...
    assert condition_forall.get_kind() == "forall"
    assert effect_forall.get_kind() == "forall"
    assert effect_probabilistic.get_kind() == "probabilistic"
    assert multi.get_kind() == "multi"
    assert len(effect_probabilistic.get_value().get_alternatives()) == 2
    assert domain.get_axioms()[0].get_condition().get_kind() == "forall"
    assert task.get_goal().get_kind() == "forall"
