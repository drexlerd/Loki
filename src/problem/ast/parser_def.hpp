...
const auto basic_function_term_def =
    ((lit('(') > domain::function_symbol() > *domain::name()) > lit(')')) | (domain::function_symbol() > x3::attr(std::vector<domain::ast::Name> {}));
...
const auto initial_element_numeric_fluents_total_cost_def = (lit('(') >> lit('=') >> lit('(') >> domain::function_symbol_total_cost()) > lit(')')
                                                            > domain::number() > lit(')');
...
const auto metric_function_expression_binary_operator_def = (lit('(') >> domain::binary_operator() >> metric_function_expression >> metric_function_expression)
                                                            > lit(')');
const auto metric_function_expression_multi_operator_def = (lit('(') >> domain::multi_operator() >> metric_function_expression >> +metric_function_expression)
                                                           > lit(')');
...
const auto preference_constraint_goal_descriptor_forall_def = (lit('(') >> domain::keyword_lit("forall")) > domain::typed_list_of_variables()
                                                              > preference_constraint_goal_descriptor > lit(')');
const auto preference_constraint_goal_descriptor_preference_def = (lit('(') >> domain::keyword_lit("preference") > (-domain::preference_name()))
                                                                  > domain::constraint_goal_descriptor() > lit(')');
...
const auto metric_specification_def = (lit('(') >> domain::keyword_lit(":metric")) > (metric_specification_total_cost | metric_specification_general)
                                      > lit(')');

const auto problem_def = lit('(') > domain::define_keyword() > problem_name > domain_name > -domain::requirements() > -objects > initial > goal > -constraints
                         > -metric_specification > lit(')');

BOOST_SPIRIT_DEFINE(basic_function_term)

BOOST_SPIRIT_DEFINE(atomic_formula_of_names_predicate, atomic_formula_of_names_equality, atomic_formula_of_names, atom, negated_atom, literal)
...
BOOST_SPIRIT_DEFINE(problem_keyword, problem_name, domain_name, objects, initial, goal, constraints, metric_specification, problem)
