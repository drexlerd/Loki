...
ProblemParser::ProblemParser(const fs::path& file_path, const DomainParser& domain_parser) {
    ...
    error_handler_type problem_error_handler(problem_source.begin(), problem_source.end(), *problem_error_stream, file_path);
    ...
    problem::Context problem_context(std::move(problem_error_stream), std::move(problem_error_handler), domain_parser.get_context());
