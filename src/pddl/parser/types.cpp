...
TypeReferenceTypeVisitor<C>::TypeReferenceTypeVisitor(const C& context_) : context(context_)
{
}

template<ParsingContext C>
TypeList TypeReferenceTypeVisitor<C>::operator()(const ast::TypeObject&)
{
...
template<ParsingContext C>
TypeList TypeReferenceTypeVisitor<C>::operator()(const ast::TypeNumber&)
{
...
template<ParsingContext C>
TypeList TypeReferenceTypeVisitor<C>::operator()(const ast::Name& node)
{
...
template<ParsingContext C>
TypeList TypeReferenceTypeVisitor<C>::operator()(const ast::TypeEither& node)
{
...
template struct TypeReferenceTypeVisitor<DomainParsingContext>;
template struct TypeReferenceTypeVisitor<ProblemParsingContext>;

/* TypeDeclarationTypedListOfNamesVisitor */

CollectParentTypesHierarchyVisitor::CollectParentTypesHierarchyVisitor(DomainParsingContext& context_,
...
std::unordered_set<std::string> CollectParentTypesHierarchyVisitor::operator()(const ast::TypeEither& node)
{
    ...
    {
        auto visitor = CollectParentTypesHierarchyVisitor(context, type_last_occurrence);
        const auto nested_type_names = boost::apply_visitor(visitor, type_node);
...
CollectTypesHierarchyVisitor::CollectTypesHierarchyVisitor(DomainParsingContext& context_,
...
void CollectTypesHierarchyVisitor::operator()(const ast::TypedListOfNamesRecursively& node)
{
    ...
    auto visitor = CollectParentTypesHierarchyVisitor(context, type_last_occurrence);
    const auto parent_types = boost::apply_visitor(visitor, node.type);
    ...
    auto visitor2 = CollectTypesHierarchyVisitor(context, child_types, type_last_occurrence);
    boost::apply_visitor(visitor2, node.typed_list_of_names.get());
}
...
TypeList parse(const ast::Types& types_node, DomainParsingContext& context)
{
