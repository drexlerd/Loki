...
pddl::RequirementEnumSet parse(const ast::RequirementFluents&, Context& context)
{
    ...
    return { pddl::RequirementEnum::FLUENTS, pddl::RequirementEnum::OBJECT_FLUENTS, pddl::RequirementEnum::NUMERIC_FLUENTS };
}
...
{
    throw UnsupportedRequirementError(pddl::RequirementEnum::DURATIVE_ACTIONS, context.scopes.get_error_handler()(node, ""));
...
{
    throw UnsupportedRequirementError(pddl::RequirementEnum::TIMED_INITIAL_LITERALS, context.scopes.get_error_handler()(node, ""));

    // Track
    context.references.track(pddl::RequirementEnum::TIMED_INITIAL_LITERALS);
    context.references.track(pddl::RequirementEnum::DURATIVE_ACTIONS);
    return { pddl::RequirementEnum::TIMED_INITIAL_LITERALS, pddl::RequirementEnum::DURATIVE_ACTIONS };
}
...
{
    throw UnsupportedRequirementError(pddl::RequirementEnum::CONSTRAINTS, context.scopes.get_error_handler()(node, ""));
...
pddl::RequirementEnumSet parse(const ast::Requirement& node, Context& context) { return boost::apply_visitor(RequirementVisitor(context), node); }
