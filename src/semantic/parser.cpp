...
Parser::Parser(const std::string& domain_source, parser::ParserOptions options) : m_options(options), m_storage(std::make_shared<detail::TranslationStorage>(0))
{
    m_object_type = intern_type("object", {});
    m_number_type = intern_type("number", {});

    auto first = domain_source.cbegin();
    parser::ErrorHandlerType error_handler(first, domain_source.cend(), std::cerr);
...
formalism::DomainView Parser::parse_domain_ast(const ast::Domain& domain)
{
...
cista::offset::string Parser::to_cista(const std::string& text) { return cista::offset::string(text); }
const formalism::Repository& Parser::repo() const noexcept { return m_storage->repository; }
void Parser::rebuild_domain_symbols()
{
    m_types.clear();
    m_objects.clear();
    m_predicates.clear();
    ...
    {
        if (auto [it, inserted] = m_types.emplace(std::string(type.get_name()), type); !inserted)
            it->second = type;
    ...
    {
        m_objects.emplace(std::string(object.get_name()), object);
    ...
    {
        m_functions.emplace(std::string(function.get_name()), function);
...
formalism::TaskView Parser::canonicalize_task(formalism::TaskView task, const std::shared_ptr<detail::TranslationStorage>& domain_storage)
{
...
void Parser::remember_requirement(formalism::RequirementKind kind)
{
...
void Parser::require_typing_if_needed(const boost::optional<ast::TypeExpression>& type, const boost::spirit::x3::position_tagged& node) const
{
    if (type)
...
ygg::Index<formalism::Type> Parser::intern_type(const std::string& name, ygg::IndexList<formalism::Type> bases)
{
    ...
    if (auto [it, inserted] = m_types.emplace(k, view); !inserted)
        it->second = view;
...
ygg::IndexList<formalism::Type> Parser::parse_type_expression(const ast::TypeExpression& type)
{
    return boost::apply_visitor([&](const auto& node) { return parse_type_expression_node(node); }, type);
}
ygg::IndexList<formalism::Type> Parser::parse_type_expression_node(const ast::EitherType& node)
{
...
ygg::IndexList<formalism::Parameter> Parser::parse_parameters(const std::vector<ast::TypedVariable>& nodes)
{
    ...
    for (const auto& node : nodes)
    {
        ...
        if (!m_variable_scopes.empty())
            m_variable_scopes.back().emplace(name, variable);
...
ygg::IndexList<formalism::FunctionSkeleton> Parser::parse_functions(const std::vector<ast::FunctionDeclaration>& nodes)
{
...
formalism::PredicateView Parser::equality_predicate(const ast::Identifier& identifier, size_t arity)
{
    ...
    m_predicates.emplace(name, view);
    return view;
}
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionLiteral& node)
{
    return wrap_condition(formalism::get_or_create<formalism::ConditionLiteral>(repo(), parse_literal(node.literal)).get_index());
}
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionOr& node)
{
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionImply& node)
{
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionForall& node)
{
...
ygg::Index<formalism::FunctionTerm> Parser::parse_function_term(const ast::FunctionTerm& node)
{
...
ygg::Index<formalism::FunctionExpression> Parser::parse_function_expression_node(const ast::FunctionExpressionFunction& node)
{
    return wrap_function_expression(parse_function_term(node.term));
}
ygg::Index<formalism::FunctionExpression> Parser::parse_function_expression_node(const ast::FunctionExpressionBinary& node)
{
...
ygg::Index<formalism::Effect> Parser::parse_effect(const ast::Effect& effect)
{
    return boost::apply_visitor([&](const auto& node) { return parse_effect_node(node); }, effect);
}
ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectLiteral& node)
{
    return wrap_effect(formalism::get_or_create<formalism::EffectLiteral>(repo(), parse_literal(node.literal)).get_index());
}
ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectNumeric& node)
{
...
ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectWhen& node)
{
...
ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectProbabilistic& node)
{
...
ygg::Index<formalism::Axiom> Parser::parse_axiom(const ast::Axiom& node)
{
    m_variable_scopes.emplace_back();
...
void Parser::parse_initial_element(const ast::InitialFunctionValue& value,
...
formalism::BinaryComparator Parser::comparator(const ast::ConditionNumericConstraint& node) const
{
...
formalism::MultiArithmeticOperator Parser::multi_operator(std::string op)
{
    return key(std::move(op)) == "*" ? formalism::MultiArithmeticOperator::Multiply : formalism::MultiArithmeticOperator::Add;
}

formalism::TaskView Parser::parse_task_ast(const ast::Task& task)
{
    if (task.domain_name.text != m_domain_name)
...
Parser::ErrorHandlerScope::ErrorHandlerScope(Parser& parser_, const parser::ErrorHandlerType& error_handler) :
    parser(parser_),
    previous(parser_.m_error_handler)
{
    parser.m_error_handler = &error_handler;
}

Parser::ErrorHandlerScope::~ErrorHandlerScope() { parser.m_error_handler = previous; }

void Parser::canonicalize_domain(formalism::DomainView domain)
{
...
void Parser::clear_domain_symbols()
{
...
std::string Parser::requirement_name(formalism::RequirementKind kind)
{
    switch (kind)
    {
...
void Parser::remember_adl_requirements()
{
...
ygg::IndexList<formalism::Requirement> Parser::parse_requirements(const std::vector<ast::Requirement>& nodes)
{
...
ygg::IndexList<formalism::Type> Parser::parse_types(const std::vector<ast::TypedName>& nodes)
{
    auto result = ygg::IndexList<formalism::Type> {};
    if (!nodes.empty())
...
ygg::IndexList<formalism::Type> Parser::parse_type_expression_node(const ast::TypeReference& node)
{
    auto result = ygg::IndexList<formalism::Type> {};
...
ygg::IndexList<formalism::Predicate> Parser::parse_predicates(const std::vector<ast::PredicateDeclaration>& nodes)
{
...
ygg::IndexList<formalism::Term> Parser::parse_terms(const std::vector<ast::Term>& nodes)
{
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionAnd& node)
{
    auto list = ygg::IndexList<formalism::Condition> {};
    for (const auto& child : node.conditions)
        list.push_back(parse_condition(child.get()));
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionNot& node)
{
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionExists& node)
{
...
ygg::Index<formalism::Condition> Parser::parse_condition_node(const ast::ConditionNumericConstraint& node)
{
    require_requirement(formalism::RequirementKind::NumericFluents, node);
...
ygg::Index<formalism::FunctionExpression> Parser::parse_function_expression(const ast::FunctionExpression& expression)
{
    return boost::apply_visitor([&](const auto& node) { return parse_function_expression_node(node); }, expression);
}

ygg::Index<formalism::FunctionExpression> Parser::parse_function_expression_node(const ast::FunctionExpressionNumber& node)
{
    return wrap_function_expression(formalism::get_or_create<formalism::FunctionExpressionNumber>(repo(), node.value).get_index());
}

ygg::Index<formalism::FunctionExpression> Parser::parse_function_expression_node(const ast::FunctionExpressionUnary& node)
{
...
ygg::Index<formalism::FunctionExpression> Parser::parse_function_expression_node(const ast::FunctionExpressionMulti& node)
{
...
ygg::Index<formalism::Effect> Parser::wrap_effect(ygg::Data<formalism::Effect>::Variant value)
{
    return formalism::get_or_create<formalism::Effect>(repo(), std::move(value)).get_index();
}

ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectAnd& node)
{
...
ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectForall& node)
{
    require_requirement(formalism::RequirementKind::UniversalPreconditions, node);
    m_variable_scopes.emplace_back();
    auto parameters = parse_parameters(node.parameters);
...
ygg::Index<formalism::Effect> Parser::parse_effect_node(const ast::EffectOneOf& node)
{
...
void Parser::parse_initial_element(const ast::Literal& literal, ygg::IndexList<formalism::Literal>& literals, ygg::IndexList<formalism::InitialFunctionValue>&)
{
    literals.push_back(parse_literal(literal));
}

ygg::Index<formalism::Metric> Parser::parse_metric(const ast::Metric& node)
{
    require_requirement(formalism::RequirementKind::NumericFluents, node);
...
formalism::BinaryArithmeticOperator Parser::binary_operator(std::string op)
{
    op = key(std::move(op));
    if (op == "/")
...
formalism::NumericEffectOperator Parser::numeric_effect_operator(const ast::EffectNumeric& node) const
{
    auto op = key(node.op);
    if (op == "assign")
