...
#ifndef LOKI2_SEMANTIC_TRANSLATOR_CANONICAL_COPY_TRANSLATOR_HPP_
#define LOKI2_SEMANTIC_TRANSLATOR_CANONICAL_COPY_TRANSLATOR_HPP_

#include "loki2/semantic/translator/common.hpp"
...
private:
    ...
    ygg::Index<pddl::Requirement> copy(ygg::Index<pddl::Requirement> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.requirement(m_storage->repository, repository[source].kind).get_index();
    ...
    ygg::Index<pddl::Type> copy(ygg::Index<pddl::Type> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.type(m_storage->repository, data.name, copy_list(data.bases, repository)).get_index();
    ...
    ygg::Index<pddl::Object> copy(ygg::Index<pddl::Object> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.object(m_storage->repository, data.name, copy_list(data.types, repository)).get_index();
    ...
    ygg::Index<pddl::Variable> copy(ygg::Index<pddl::Variable> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.variable(m_storage->repository, repository[source].name).get_index();
    ...
    ygg::Index<pddl::Parameter> copy(ygg::Index<pddl::Parameter> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.parameter(m_storage->repository, copy(data.variable, repository), copy_list(data.types, repository)).get_index();
    ...
    ygg::Index<pddl::Predicate> copy(ygg::Index<pddl::Predicate> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.predicate(m_storage->repository, data.name, copy_list(data.parameters, repository)).get_index();
    ...
    ygg::Index<pddl::FunctionSkeleton> copy(ygg::Index<pddl::FunctionSkeleton> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.function_skeleton(m_storage->repository, data.name, copy_list(data.parameters, repository), copy(data.type, repository)).get_index();
    ...
    ygg::Index<pddl::Term> copy(ygg::Index<pddl::Term> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.term(m_storage->repository, std::move(value)).get_index();
    ...
    ygg::Index<pddl::Atom> copy(ygg::Index<pddl::Atom> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.atom(m_storage->repository, copy(data.predicate, repository), copy_list(data.terms, repository)).get_index();
    ...
    ygg::Index<pddl::Literal> copy(ygg::Index<pddl::Literal> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.literal(m_storage->repository, data.positive, copy(data.atom, repository)).get_index();
    ...
    ygg::Index<pddl::FunctionTerm> copy(ygg::Index<pddl::FunctionTerm> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.function_term(m_storage->repository, copy(data.function, repository), copy_list(data.terms, repository)).get_index();
    ...
    ygg::Index<pddl::MultiFunctionExpression> copy(ygg::Index<pddl::MultiFunctionExpression> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.multi_function_expression(m_storage->repository, data.op, copy_list(data.expressions, repository)).get_index();
    ...
    ygg::Index<pddl::ConditionAnd> copy(ygg::Index<pddl::ConditionAnd> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.condition_and(m_storage->repository, copy_list(repository[source].conditions, repository)).get_index();
    ...
    ygg::Index<pddl::ConditionOr> copy(ygg::Index<pddl::ConditionOr> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.condition_or(m_storage->repository, copy_list(repository[source].conditions, repository)).get_index();
    ...
    ygg::Index<pddl::ConditionExists> copy(ygg::Index<pddl::ConditionExists> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.condition_exists(m_storage->repository, copy_list(data.parameters, repository), copy(data.condition, repository)).get_index();
    ...
    ygg::Index<pddl::ConditionForall> copy(ygg::Index<pddl::ConditionForall> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.condition_forall(m_storage->repository, copy_list(data.parameters, repository), copy(data.condition, repository)).get_index();
    ...
    ygg::Index<pddl::Condition> copy(ygg::Index<pddl::Condition> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.condition(m_storage->repository, std::move(value)).get_index();
    ...
    ygg::Index<pddl::EffectAnd> copy(ygg::Index<pddl::EffectAnd> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.effect_and(m_storage->repository, copy_list(repository[source].effects, repository)).get_index();
    ...
    ygg::Index<pddl::EffectNumeric> copy(ygg::Index<pddl::EffectNumeric> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.effect_numeric(m_storage->repository, data.op, copy(data.function, repository), copy_list(data.terms, repository), copy(data.expression, repository)).get_index();
    ...
    ygg::Index<pddl::EffectForall> copy(ygg::Index<pddl::EffectForall> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.effect_forall(m_storage->repository, copy_list(data.parameters, repository), copy(data.effect, repository)).get_index();
    ...
    ygg::Index<pddl::EffectOneOf> copy(ygg::Index<pddl::EffectOneOf> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.effect_one_of(m_storage->repository, copy_list(repository[source].effects, repository)).get_index();
    ...
    ygg::Index<pddl::EffectProbabilistic> copy(ygg::Index<pddl::EffectProbabilistic> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.effect_probabilistic(m_storage->repository, copy_list(repository[source].alternatives, repository)).get_index();
    ...
    ygg::Index<pddl::Effect> copy(ygg::Index<pddl::Effect> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.effect(m_storage->repository, std::move(value)).get_index();
    ...
    ygg::Index<pddl::Action> copy(ygg::Index<pddl::Action> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.action(m_storage->repository, data.name, copy_list(data.parameters, repository), copy_optional(data.precondition, repository), copy_optional(data.effect, repository)).get_index();
    ...
    ygg::Index<pddl::Axiom> copy(ygg::Index<pddl::Axiom> source, const pddl::Repository& repository)
    {
        ...
        const auto& data = repository[source];
        out = m_storage->builder.axiom(m_storage->repository, copy_list(data.parameters, repository), copy(data.head, repository), copy(data.condition, repository)).get_index();
    ...
    ygg::Index<pddl::Domain> copy(ygg::Index<pddl::Domain> source, const pddl::Repository& repository)
    {
        ...
        out = m_storage->builder.domain(m_storage->repository, std::move(copied)).get_index();
