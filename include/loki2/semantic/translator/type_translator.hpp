...
#ifndef LOKI2_SEMANTIC_TRANSLATOR_TYPE_TRANSLATOR_HPP_
#define LOKI2_SEMANTIC_TRANSLATOR_TYPE_TRANSLATOR_HPP_
...
ygg::Index<pddl::Parameter> TypeTranslator<Derived>::maybe_strip_parameter(ygg::Index<pddl::Parameter> parameter)
{
    ...
    return this->m_storage->builder.parameter(this->m_storage->repository, data.variable, {}).get_index();
}
...
ygg::Index<pddl::Object> TypeTranslator<Derived>::maybe_strip_object(ygg::Index<pddl::Object> object)
{
    ...
    return this->m_storage->builder.object(this->m_storage->repository, data.name, {}).get_index();
}
...
ygg::Index<pddl::Predicate> TypeTranslator<Derived>::type_predicate(ygg::Index<pddl::Type> type)
{
    ...
    const auto variable = this->m_storage->builder.variable(this->m_storage->repository, cista::offset::string("arg")).get_index();
    auto parameter_types = ygg::IndexList<pddl::Type> {};
    if (!this->m_remove_typing)
        parameter_types.push_back(type);
    parameters.push_back(this->m_storage->builder.parameter(this->m_storage->repository, variable, std::move(parameter_types)).get_index());
    const auto predicate = this->m_storage->builder.predicate(this->m_storage->repository, cista::offset::string(name), std::move(parameters)).get_index();
    this->m_type_predicates.emplace(type.get_value(), predicate);
    return predicate;
}
...
ygg::Index<pddl::Literal> TypeTranslator<Derived>::type_literal(ygg::Index<pddl::Type> type, ygg::Index<pddl::Term> term)
{
    ...
    const auto atom = this->m_storage->builder.atom(this->m_storage->repository, this->self().type_predicate(type), std::move(terms)).get_index();
...
{
    const auto term = this->m_storage->builder.term(this->m_storage->repository, ygg::Data<pddl::Term>::Variant(variable)).get_index();
    return this->self().wrap_condition(this->m_storage->builder.condition_literal(this->m_storage->repository, this->self().type_literal(type, term)).get_index());
}
