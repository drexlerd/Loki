...
#ifndef LOKI2_SEMANTIC_TRANSLATOR_EFFECT_TRANSLATOR_HPP_
#define LOKI2_SEMANTIC_TRANSLATOR_EFFECT_TRANSLATOR_HPP_
...
ygg::Index<pddl::EffectLiteral> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectLiteral> source, const pddl::Repository& repository)
{
    ...
    out = this->m_storage->builder.effect_literal(this->m_storage->repository, this->self().copy(repository[source].literal, repository)).get_index();
...
ygg::Index<pddl::EffectAnd> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectAnd> source, const pddl::Repository& repository)
{
    ...
    out = this->m_storage->builder.effect_and(this->m_storage->repository, this->self().template copy_list<pddl::Effect>(repository[source].effects, repository)).get_index();
...
ygg::Index<pddl::EffectNumeric> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectNumeric> source, const pddl::Repository& repository)
{
    ...
    out = this->m_storage->builder.effect_numeric(this->m_storage->repository, data.op, this->self().copy(data.function, repository), this->self().template copy_list<pddl::Term>(data.terms, repository), this->self().copy(data.expression, repository)).get_index();
...
ygg::Index<pddl::EffectForall> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectForall> source, const pddl::Repository& repository)
{
    ...
    if (!guard.empty())
    {
        ...
        effect = this->self().wrap_effect(ygg::Data<pddl::Effect>::Variant(this->m_storage->builder.effect_when(this->m_storage->repository, condition, effect).get_index()));
    }
    out = this->m_storage->builder.effect_forall(this->m_storage->repository, this->self().maybe_strip_parameters(parameters), effect).get_index();
...
ygg::Index<pddl::EffectWhen> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectWhen> source, const pddl::Repository& repository)
{
    ...
    const auto& data = repository[source];
    out = this->m_storage->builder.effect_when(this->m_storage->repository, this->self().copy(data.condition, repository), this->self().copy(data.effect, repository)).get_index();
...
ygg::Index<pddl::EffectOneOf> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectOneOf> source, const pddl::Repository& repository)
{
    ...
    out = this->m_storage->builder.effect_one_of(this->m_storage->repository, this->self().template copy_list<pddl::Effect>(repository[source].effects, repository)).get_index();
...
ygg::Index<pddl::EffectProbabilisticAlternative> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectProbabilisticAlternative> source, const pddl::Repository& repository)
{
    ...
    const auto& data = repository[source];
    out = this->m_storage->builder.effect_probabilistic_alternative(this->m_storage->repository, data.probability, this->self().copy(data.effect, repository)).get_index();
...
ygg::Index<pddl::EffectProbabilistic> EffectTranslator<Derived>::copy(ygg::Index<pddl::EffectProbabilistic> source, const pddl::Repository& repository)
{
    ...
    out = this->m_storage->builder.effect_probabilistic(this->m_storage->repository, this->self().template copy_list<pddl::EffectProbabilisticAlternative>(repository[source].alternatives, repository)).get_index();
...
ygg::Index<pddl::Effect> EffectTranslator<Derived>::copy(ygg::Index<pddl::Effect> source, const pddl::Repository& repository)
{
    ...
    out = this->self().normalize_effect(this->m_storage->builder.effect(this->m_storage->repository, std::move(value)).get_index());
