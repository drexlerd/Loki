...
private:
    ...
    {
        if (auto mapped = find_mapped(m_storage->requirements, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->types, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->objects, source)) return *mapped;
        const auto& data = repository[source];
    ...
    {
        if (auto mapped = find_mapped(m_storage->variables, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->parameters, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::Parameter>(m_storage->repository, as_index(copy(data.variable, repository)), copy_list(data.types, repository));
    ...
    {
        if (auto mapped = find_mapped(m_storage->predicates, source)) return *mapped;
        const auto& data = repository[source];
    ...
    {
        if (auto mapped = find_mapped(m_storage->functions, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::FunctionSkeleton>(m_storage->repository, data.name, copy_list(data.parameters, repository), as_index(copy(data.type, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->terms, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->atoms, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::Atom>(m_storage->repository, as_index(copy(data.predicate, repository)), copy_list(data.terms, repository));
    ...
    {
        if (auto mapped = find_mapped(m_storage->literals, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::Literal>(m_storage->repository, data.positive, as_index(copy(data.atom, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->numbers, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->function_terms, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::FunctionTerm>(m_storage->repository, as_index(copy(data.function, repository)), copy_list(data.terms, repository));
    ...
    {
        if (auto mapped = find_mapped(m_storage->unary_expressions, source)) return *mapped;
        const auto& data = repository[source];
    ...
    {
        if (auto mapped = find_mapped(m_storage->binary_expressions, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::BinaryFunctionExpression>(m_storage->repository, data.op, as_index(copy(data.left, repository)), as_index(copy(data.right, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->multi_expressions, source)) return *mapped;
        const auto& data = repository[source];
    ...
    {
        if (auto mapped = find_mapped(m_storage->function_expressions, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_literals, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_ands, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_ors, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_nots, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_implies, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::ConditionImply>(m_storage->repository, as_index(copy(data.left, repository)), as_index(copy(data.right, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_exists, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::ConditionExists>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.condition, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_foralls, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::ConditionForall>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.condition, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->condition_numeric_constraints, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::ConditionNumericConstraint>(m_storage->repository, data.comparator, as_index(copy(data.left, repository)), as_index(copy(data.right, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->conditions, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_literals, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_ands, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_numerics, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::EffectNumeric>(m_storage->repository, data.op, as_index(copy(data.function, repository)), copy_list(data.terms, repository), as_index(copy(data.expression, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_foralls, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::EffectForall>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.effect, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_whens, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::EffectWhen>(m_storage->repository, as_index(copy(data.condition, repository)), as_index(copy(data.effect, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_one_ofs, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_probabilistic_alternatives, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::EffectProbabilisticAlternative>(m_storage->repository, data.probability, as_index(copy(data.effect, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->effect_probabilistics, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->effects, source)) return *mapped;
    ...
    {
        if (auto mapped = find_mapped(m_storage->actions, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::Action>(m_storage->repository, data.name, copy_list(data.parameters, repository), copy_optional(data.precondition, repository), copy_optional(data.effect, repository));
    ...
    {
        if (auto mapped = find_mapped(m_storage->axioms, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::Axiom>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.head, repository)), as_index(copy(data.condition, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->metrics, source)) return *mapped;
        const auto& data = repository[source];
    ...
    {
        if (auto mapped = find_mapped(m_storage->initial_function_values, source)) return *mapped;
        const auto& data = repository[source];
        auto out = formalism::get_or_create<formalism::InitialFunctionValue>(m_storage->repository, as_index(copy(data.function, repository)), as_index(copy(data.value, repository)));
    ...
    {
        if (auto mapped = find_mapped(m_storage->domains, source)) return *mapped;
        const auto& data = repository[source];
        auto copied = data;
        copied.index = {};
