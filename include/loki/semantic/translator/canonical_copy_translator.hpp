...
private:
    ...
    formalism::RequirementView copy(ygg::Index<formalism::Requirement> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->requirements, source, out)) return ygg::make_view(out, m_storage->repository);
        ...
        return ygg::make_view(out, m_storage->repository);
    }
    ...
    formalism::TypeView copy(ygg::Index<formalism::Type> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->types, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::ObjectView copy(ygg::Index<formalism::Object> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->objects, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
    ...
    formalism::VariableView copy(ygg::Index<formalism::Variable> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->variables, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::ParameterView copy(ygg::Index<formalism::Parameter> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->parameters, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::Parameter>(m_storage->repository, as_index(copy(data.variable, repository)), copy_list(data.types, repository)).get_index();
    ...
    formalism::PredicateView copy(ygg::Index<formalism::Predicate> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->predicates, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
    ...
    formalism::FunctionSkeletonView copy(ygg::Index<formalism::FunctionSkeleton> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->functions, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::FunctionSkeleton>(m_storage->repository, data.name, copy_list(data.parameters, repository), as_index(copy(data.type, repository))).get_index();
    ...
    formalism::TermView copy(ygg::Index<formalism::Term> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->terms, source, out)) return ygg::make_view(out, m_storage->repository);
        auto value = std::visit([&](const auto& arg) -> ygg::Data<formalism::Term>::Variant { return ygg::Data<formalism::Term>::Variant(as_index(copy(arg, repository))); }, repository[source].value);
    ...
    formalism::AtomView copy(ygg::Index<formalism::Atom> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->atoms, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::Atom>(m_storage->repository, as_index(copy(data.predicate, repository)), copy_list(data.terms, repository)).get_index();
    ...
    formalism::LiteralView copy(ygg::Index<formalism::Literal> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->literals, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::Literal>(m_storage->repository, data.positive, as_index(copy(data.atom, repository))).get_index();
    ...
    formalism::FunctionExpressionNumberView copy(ygg::Index<formalism::FunctionExpressionNumber> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->numbers, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::FunctionTermView copy(ygg::Index<formalism::FunctionTerm> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->function_terms, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::FunctionTerm>(m_storage->repository, as_index(copy(data.function, repository)), copy_list(data.terms, repository)).get_index();
    ...
    formalism::UnaryFunctionExpressionView copy(ygg::Index<formalism::UnaryFunctionExpression> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->unary_expressions, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::UnaryFunctionExpression>(m_storage->repository, data.op, as_index(copy(data.expression, repository))).get_index();
    ...
    formalism::BinaryFunctionExpressionView copy(ygg::Index<formalism::BinaryFunctionExpression> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->binary_expressions, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::BinaryFunctionExpression>(m_storage->repository, data.op, as_index(copy(data.left, repository)), as_index(copy(data.right, repository))).get_index();
    ...
    formalism::MultiFunctionExpressionView copy(ygg::Index<formalism::MultiFunctionExpression> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->multi_expressions, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
    ...
    formalism::FunctionExpressionView copy(ygg::Index<formalism::FunctionExpression> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->function_expressions, source, out)) return ygg::make_view(out, m_storage->repository);
        auto value = std::visit([&](const auto& arg) -> ygg::Data<formalism::FunctionExpression>::Variant { return ygg::Data<formalism::FunctionExpression>::Variant(as_index(copy(arg, repository))); }, repository[source].value);
    ...
    formalism::ConditionLiteralView copy(ygg::Index<formalism::ConditionLiteral> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_literals, source, out)) return ygg::make_view(out, m_storage->repository);
        out = formalism::get_or_create<formalism::ConditionLiteral>(m_storage->repository, as_index(copy(repository[source].literal, repository))).get_index();
    ...
    formalism::ConditionAndView copy(ygg::Index<formalism::ConditionAnd> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_ands, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::ConditionOrView copy(ygg::Index<formalism::ConditionOr> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_ors, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::ConditionNotView copy(ygg::Index<formalism::ConditionNot> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_nots, source, out)) return ygg::make_view(out, m_storage->repository);
        out = formalism::get_or_create<formalism::ConditionNot>(m_storage->repository, as_index(copy(repository[source].condition, repository))).get_index();
    ...
    formalism::ConditionImplyView copy(ygg::Index<formalism::ConditionImply> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_implies, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::ConditionImply>(m_storage->repository, as_index(copy(data.left, repository)), as_index(copy(data.right, repository))).get_index();
    ...
    formalism::ConditionExistsView copy(ygg::Index<formalism::ConditionExists> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_exists, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::ConditionExists>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.condition, repository))).get_index();
    ...
    formalism::ConditionForallView copy(ygg::Index<formalism::ConditionForall> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_foralls, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::ConditionForall>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.condition, repository))).get_index();
    ...
    formalism::ConditionNumericConstraintView copy(ygg::Index<formalism::ConditionNumericConstraint> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->condition_numeric_constraints, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::ConditionNumericConstraint>(m_storage->repository, data.comparator, as_index(copy(data.left, repository)), as_index(copy(data.right, repository))).get_index();
    ...
    formalism::ConditionView copy(ygg::Index<formalism::Condition> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->conditions, source, out)) return ygg::make_view(out, m_storage->repository);
        auto value = std::visit([&](const auto& arg) -> ygg::Data<formalism::Condition>::Variant { return ygg::Data<formalism::Condition>::Variant(as_index(copy(arg, repository))); }, repository[source].value);
    ...
    formalism::EffectLiteralView copy(ygg::Index<formalism::EffectLiteral> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_literals, source, out)) return ygg::make_view(out, m_storage->repository);
        out = formalism::get_or_create<formalism::EffectLiteral>(m_storage->repository, as_index(copy(repository[source].literal, repository))).get_index();
    ...
    formalism::EffectAndView copy(ygg::Index<formalism::EffectAnd> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_ands, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::EffectNumericView copy(ygg::Index<formalism::EffectNumeric> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_numerics, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::EffectNumeric>(m_storage->repository, data.op, as_index(copy(data.function, repository)), copy_list(data.terms, repository), as_index(copy(data.expression, repository))).get_index();
    ...
    formalism::EffectForallView copy(ygg::Index<formalism::EffectForall> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_foralls, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::EffectForall>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.effect, repository))).get_index();
    ...
    formalism::EffectWhenView copy(ygg::Index<formalism::EffectWhen> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_whens, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::EffectWhen>(m_storage->repository, as_index(copy(data.condition, repository)), as_index(copy(data.effect, repository))).get_index();
    ...
    formalism::EffectOneOfView copy(ygg::Index<formalism::EffectOneOf> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_one_ofs, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::EffectProbabilisticAlternativeView copy(ygg::Index<formalism::EffectProbabilisticAlternative> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_probabilistic_alternatives, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::EffectProbabilisticAlternative>(m_storage->repository, data.probability, as_index(copy(data.effect, repository))).get_index();
    ...
    formalism::EffectProbabilisticView copy(ygg::Index<formalism::EffectProbabilistic> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effect_probabilistics, source, out)) return ygg::make_view(out, m_storage->repository);
    ...
    formalism::EffectView copy(ygg::Index<formalism::Effect> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->effects, source, out)) return ygg::make_view(out, m_storage->repository);
        auto value = std::visit([&](const auto& arg) -> ygg::Data<formalism::Effect>::Variant { return ygg::Data<formalism::Effect>::Variant(as_index(copy(arg, repository))); }, repository[source].value);
    ...
    formalism::ActionView copy(ygg::Index<formalism::Action> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->actions, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
    ...
    formalism::AxiomView copy(ygg::Index<formalism::Axiom> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->axioms, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::Axiom>(m_storage->repository, copy_list(data.parameters, repository), as_index(copy(data.head, repository)), as_index(copy(data.condition, repository))).get_index();
    ...
    formalism::MetricView copy(ygg::Index<formalism::Metric> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->metrics, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::Metric>(m_storage->repository, data.minimize, as_index(copy(data.expression, repository))).get_index();
    ...
    formalism::InitialFunctionValueView copy(ygg::Index<formalism::InitialFunctionValue> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->initial_function_values, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        out = formalism::get_or_create<formalism::InitialFunctionValue>(m_storage->repository, as_index(copy(data.function, repository)), as_index(copy(data.value, repository))).get_index();
    ...
    formalism::DomainView copy(ygg::Index<formalism::Domain> source, const formalism::Repository& repository)
    {
        ...
        if (find_mapped(m_storage->domains, source, out)) return ygg::make_view(out, m_storage->repository);
        const auto& data = repository[source];
        auto copied = data;
        copied.index = {};
